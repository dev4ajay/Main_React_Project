...
export default function Trips() {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await axiosInstance.get('/api/trips/trip?tripType=round_trip');
                ...
                <Grid container>
                    <Grid item xs={12}>
                        <div className='bg-[#ecf2ff] m-5 p-7 rounded-2xl'>
                            <h1 className='text-xl font-extrabold text-[#34609d] '>Round Trip</h1>
                            <p className='py-2 font-semibold text-xs text-[#34609d]'>Home <FiberManualRecordIcon className='text-sm px-1' /> Destinations
                            <FiberManualRecordIcon className='text-sm px-1' />Round Trip</p>
                        </div>
                    </Grid>
                    <Grid xs={12} className='py-2 px-5'>
                        ...
                        <TextField
                            ...
                            <div className='ml-[auto] flex justify-between'>
                             <Button className='bg-[#34609d] hover:bg-[#34609d] hover:opacity-50 m-0 px-5 text-white rounded-xl' onClick={() => router.push('/admin/add-round-trip-destination')}>Add Round Trip</Button>   
                        ...
                        <div className='px-0 py-5 border-[1px] m-6 rounded-lg' >
                            ...
                            <div className='text-center py-5'>
                              <CircularProgress />
                                ...
                                rows={filteredRows}
                                headers={headers}
                                onEditClick={(row) => {router.push(`/admin/edit-roundtrip/${row?.id}`)}                  
                              } 
